...
function validateFiles(files: File[]) {
  if (!files.length) return '请选择一个笔记文件夹';
...
export default function AdminUpload() {
  ...
  async function submit(event: FormEvent) {
    ...
    setResult(null);
    try {
      setResult(await uploadFolder('note', category.trim(), files));
    } catch (cause) {
  ...
  return (
    <div className="note-admin-upload min-h-screen text-[#ddd]">
      <header className="note-admin-header note-toolbar liquid-glass h-12 flex items-center justify-between px-5">
        <NoteBrandHome />
        <div className="note-admin-header__actions flex items-center gap-4"><Link to="/">笔记首页</Link><AuthMenu compact /></div>
      </header>
      <main className="admin-upload max-w-[720px] mx-auto px-6 py-14 md:py-20">
        ...
        {result && (
          <div className="admin-upload__success" role="status">
            ...
            <p>{result.message}</p>
            <code>$env:NOTE_CONTENT_ROOT='&lt;与 Site-api 相同的内容根&gt;'; npm --prefix Note run build</code>
          </div>
        )}
      </main>
    </div>
